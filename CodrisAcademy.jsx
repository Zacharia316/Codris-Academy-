...
function OnboardingScreen({ onComplete }) {
  ...
  const testKey = async () => {
    ...
    try {
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: GROQ_MODELS[1], max_tokens: 10, messages: [{ role: "user", content: "hi" }] }),
      });
      if (res.ok) setStep(1);
      else setError("Invalid key. Please check and try again.");
    } catch {
  ...
  const steps = [
    ...
    <div key="key" style={styles.onboardCard}>
      ...
      <button style={styles.primaryBtn} onClick={testKey} disabled={testing || !apiKey}>
        {testing ? "Verifying..." : "Activate Codris →"}
      </button>
    </div>,
